...
module.exports = class ingressoController {
  ...
  // Buscar ingressos por evento
    static async getByIdEvento(req, res) {
        ...
        const query = `SELECT i.id_ingresso, i.preco, i.tipo, e.nome, e.id_evento FROM 
        ingresso i 
        JOIN evento e 
        ON i.fk_id_evento = e.id_evento
        WHERE i.fk_id_evento = ?`;
        ...
        try {
            connect.query(query, [eventoId], (err, results) => {
                ...
                if(results.length === 0){
                    return res.status(404).json({error: "Ingresso(s) não encontrado(s)"});
                };
                
                res.status(200).json({ message: "Ingressos obtidos com sucesso", ingressos: results});
            })
        } catch (error) {
